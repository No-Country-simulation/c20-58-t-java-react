    ...
    <>
      ...
      <section className="flex flex-col place-content-center items-center">
        <NombreUsuario
          ...
          role={teacher.role}
        />
        <Titulos title="Materias Dictadas" />
        <div className="flex justify-center max-w-3xl gap-[15px] flex-wrap p-4 items-center overflow-y-auto h-80 scrollbar-hide scroll-smooth bg-gradient-to-br from-blue-500 to-pink-500 rounded-xl p-8 max-sm:min-h-[50vh]">
          {stateSubject.map(({ id, subject, group }) => {
            return (
              <div
                key={id}
                className="border-2  border-green-700 p-2 rounded-lg min-h-[10rem] bg-white"
              >
                <CardsSubjectInfo
                  img={subjectImage}
                  alt="ImageSubject"
                  ...
                  subjectId={id}
                />
                <h2 className="text-lg uppercase text-azulClaroBase">
                  {subject}
                </h2>
